...
class TestDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #prop_tensor = {key: torch.tensor(value) for key, value in prop_values.items()} if prop_values else None
        #val_prop_tensor = torch.tensor([prop], dtype = torch.float)
        ...
        mask = (v_output_tensor.clone() != stoi_dict['[PAD]']).float()
        #mask = (enc_input.clone() == stoi_dict['[MASK]']).float()
        
        
        
        #prop_tensor = prop_tensor.to(torch.float32)
        

       
        return enc_input, dec_input_tensor, v_output_tensor, mask
...
'''def subs_with_inf_min(prop_tensor, prop):
    props = prop = ['MW', 'QED', 'SAS', 'TPSA', 'logP', 'HBA', 'HBD', 'NumRotBonds']
    na_props = [na_prop for na_prop in props if na_prop not in prop]
    prop_tensor.update({key: torch.tensor(-1e9) for key in na_props})
    print(sorted(prop_tensor))
...
class Encoder(nn.Module):
    ...
    def forward(self, sca, prop = None, is_train = True):                                               # enc_inputs: [batch_size, src_len]
...
#for prop
config = GPTConfig(vocab_size=98, max_len = max_len + 1, embed_dim=256, num_heads = 8, 
                dropout=0.0, acti_pdrop=0.0, ffn_pdrop=0.0, ffn_dim=512, num_props = 0,
                   scaffold=False)
...
# In[63]:
ck_path = '/home/10714016/gpt/result/combined_data/encode5_prop_pp/checkpoint_cloze_1_06.pt'
checkpoint = torch.load(ck_path)
#checkpoint["encoder.prop_nn.weight"] = torch.randn((128, 5))
#checkpoint["encoder.prop_nn.bias"] = torch.zeros(128)
...
for epoch in range(10):
    ...
    batch = 0
    for enc_input, dec_input_tensor, v_output, padding in train_dataloader:                                                           
        #outputs, enc_self_attns, dec_self_attns, cross_attns = model(enc_inputs, dec_inputs)
                                                               #size = [bsz, tgt_len, vocab_size]
        optimizer.zero_grad()  
        logits, train_loss = model(enc_input, v_output, padding)
        ...
        #print(total_cloze_correct, total_cloze_count)
    ...
    if True:
        ...
        batch = 0
        with torch.no_grad():
            for val_enc_input, val_dec_input, val_v_output, val_padding in val_dataloader:
                val_logits, val_loss = model(val_enc_input, val_v_output, val_padding)
            ...
            if good_model:
                ...
                #save_checkpoint(model, '/home/10714016/gpt/result/combined_data/encode5_prop_pp/checkpoint_cloze_1_06_smiles.pt')
        model.train()
    print('train loss:', average_train_loss, ' ', 'valid loss', average_val_loss, 'train_acc:', cloze_accuracy, 'val_acc', val_cloze_accuracy)
...
#plt.savefig('/home/10714016/gpt/result/combined_data/encode5_prop_pp/loss_cloze_1_06_smiles.jpg')
plt.show()
...
#plt.savefig('/home/10714016/gpt/result/combined_data/encode5_prop_pp/lr_cloze_1_06_smiles.jpg')
plt.show()
...
#plt.savefig('/home/10714016/gpt/result/combined_data/encode5_prop_pp/acc_cloze_1_06_smiles.jpg')
...
#losses_df.to_csv('/home/10714016/gpt/result/combined_data/encode5_prop_pp/c_loss_cloze_1_06_smiles.csv')
#acc_df.to_csv('/home/10714016/gpt/result/combined_data/encode5_prop_pp/c_acc_cloze_1_06_smiles.csv')
#lr_df.to_csv('/home/10714016/gpt/result/combined_data/encode5_prop_pp/c_lr_cloze_1_06_smiles.csv')
