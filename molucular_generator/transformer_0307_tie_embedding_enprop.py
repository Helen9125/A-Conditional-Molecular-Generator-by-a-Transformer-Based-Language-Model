...
#prop5: prop = ['MW', 'QED', 'SAS', 'TPSA', 'logP']
#prop7: prop = ['HBA', 'HBD', 'MW', 'NumRotBonds', 'QED', 'TPSA', 'logP']
...
class Model_prop(nn.Module):
    def __init__(self, config):
        super(Model_prop, self).__init__()
        self.embed = Embeddinglayer()
        self.encoder = Encoder()
        self.decoder = Decoder()
        self.proj = nn.Linear(config.embed_dim, config.vocab_size)
        
    def forward(self, enc_input, prop, dec_input, v_output, padding):
        enc_hidden_states = self.embed(prop, enc_input, is_encoder = True)
        enc_output, enc_self_attns = self.encoder(enc_input, enc_hidden_states)
        dec_hidden_states = self.embed(prop, dec_input, is_encoder = False)
        ...
        loss = smiles_loss(logits, v_output, padding)
        #reshaped: output:[bsz * tgt_len, vocab_size], v_output:[bsz * tgt_len]
        return logits, loss
...
'''ck_path_encoder = '/home/10714016/gpt/result/encode_5/checkpoint_cloze_1_0660_smiles_6_prop_6.pt'
checkpoint_encoder = torch.load(ck_path_encoder)
ck_path_decoder = '/home/10714016/gpt/result/encode_5/rdkit/checkpoint_whole_prop7.pt'
checkpoint_decoder = torch.load(ck_path_decoder)
...
#import numpy as np
#for k in list(checkpoint_decoder.keys()):
    #a = ["None" if not k.startswith("encoder") else "Exists" for k in list(checkpoint_decoder.keys())]
    #print(np.unique(a))
...
# Copy the encoder part from checkpoint_encoder
for k in list(checkpoint_encoder.keys()):
    checkpoint_new_2[k] = checkpoint_encoder[k]

checkpoint_new_2["embed.tok_emb.weight"] = checkpoint_encoder["encoder.tok_emb.weight"]
#checkpoint_new["embed.tok_emb.bias"] = checkpoint_encoder["encoder.tok_emb.bias"]

del checkpoint_new_2["encoder.tok_emb.weight"]
#del checkpoint_new_2["encoder.tok_emb.bias"]
del checkpoint_new_2["encoder.type_emb.weight"]
#del checkpoint_new_2["encoder.type_emb.bias"]
del checkpoint_new_2["prop_proj.weight"]
del checkpoint_new_2["prop_proj.bias"]

for k in list(checkpoint_decoder.keys()):
    if k.startswith("encoder"):
        del checkpoint_decoder[k]

del checkpoint_decoder["embed.tok_emb.weight"]

for k in list(checkpoint_decoder.keys()):
    checkpoint_new_2[k] = checkpoint_decoder[k]'''

ck_path = '/home/10714016/gpt/result/combined_data/whole/checkpoint_cloze_smiles.pt'
checkpoint = torch.load(ck_path)
checkpoint["embed.prop_nn.weight"] = torch.randn((config.embed_dim,config.num_props))
checkpoint["embed.prop_nn.bias"] = torch.zeros(config.embed_dim)

# In[33]:


model = Model_prop(config)
...
'''def adjusted_model(epoch, model):
    if epoch < 10:
        ...
        model.embed.tok_emb.weight.requires_grad = False

    else:
        for name, param in model.named_parameters():
            if name.startswith('encoder'):
                param.requires_grad = True
        model.embed.tok_emb.weight.requires_grad = True

    return model'''
def adjusted_model(epoch, model):
    if epoch < 5:
        for name, param in model.named_parameters():
            if name == 'embed.prop_nn.weight' or name == 'embed.prop_nn.bias':
                param.requires_grad = True
            else:
                param.requires_grad = False
    elif epoch < 10:
...
for epoch in range(50):
    ...
    batch = 0
    for enc_input, v_prop, dec_input, v_output, padding in train_dataloader:                                                           
        #outputs, enc_self_attns, dec_self_attns, cross_attns = model(enc_inputs, dec_inputs)
                                                               #size = [bsz, tgt_len, vocab_size]
        optimizer.zero_grad()  
        logits, train_loss = ad_model(enc_input, v_prop, dec_input, v_output, padding)
        total_train_loss += train_loss

        probs = F.softmax(logits, dim=-1) #size=[bsz, len, vocab_size]
        predictions = torch.argmax(probs, dim=2) #size=[bsz, len]
        mask_position = (padding != 0)
        masked_predictions = predictions[mask_position]
        masked_v_outputs = v_output[mask_position]
        
        cloze_correct = torch.sum(masked_predictions == masked_v_outputs)
        total_cloze_correct += cloze_correct.item()
        cloze_count = torch.sum(padding)
        total_cloze_count += cloze_count.item()
    ...
    cloze_accuracy = total_cloze_correct / total_cloze_count
    train_losses.append(average_train_loss.item())
    train_acc.append(cloze_accuracy)    
    ...
    if True:
        ...
        batch = 0
        with torch.no_grad():
            for val_enc_input, val_v_prop, val_dec_input, val_v_output, val_padding in val_dataloader:
                val_logits, val_loss = ad_model(
                    val_enc_input, val_v_prop, val_dec_input, val_v_output, val_padding)
                total_val_loss += val_loss
                val_probs = F.softmax(val_logits, dim=-1) #size=[bsz, len, vocab_size]
                val_predictions = torch.argmax(val_probs, dim=2) #size=[bsz, len]
                #print(val_predictions.size(), val_v_outputs.size(), val_mask.size())
                val_mask_position = (val_padding != 0)
                val_masked_predictions = val_predictions[val_mask_position]
                val_masked_v_outputs = val_v_output[val_mask_position]
                
                val_cloze_correct = torch.sum(val_masked_predictions == val_masked_v_outputs)
                val_total_cloze_correct += val_cloze_correct.item()
                val_cloze_count = torch.sum(val_padding)
                val_total_cloze_count += val_cloze_count.item()
                
                batch += 1
            average_val_loss = total_val_loss / batch
            val_cloze_accuracy = val_total_cloze_correct / val_total_cloze_count
            val_losses.append(average_val_loss.item())
            val_acc.append(val_cloze_accuracy)
                
            good_model = average_val_loss < best_loss
            if good_model:
                best_loss = average_val_loss
                print(f'Saving at epoch {epoch + 1}')
                save_checkpoint(model, '/home/10714016/gpt/result/combined_data/whole/checkpoint_cloze_smiles_enprop5.pt')
        model.train()
    print('train loss:', average_train_loss, ' ', 'valid loss', average_val_loss, 
          'train_acc:', cloze_accuracy, 'val_acc', val_cloze_accuracy)
...
losses_df.to_csv('/home/10714016/gpt/result/combined_data/whole/c_loss_cloze_smiles_enprop5.csv')
lr_df.to_csv('/home/10714016/gpt/result/combined_data/whole/c_lr_cloze_smiles_enprop5.csv')
acc_df.to_csv('/home/10714016/gpt/result/combined_data/whole/c_acc_cloze_smiles_enprop5.csv')
# In[63]:
...
plt.savefig('/home/10714016/gpt/result/combined_data/whole/loss_cloze_smiles_enprop5.jpg')
plt.show()
...
plt.savefig('/home/10714016/gpt/result/combined_data/whole/lr_cloze_smiles_enprop5.jpg')
plt.show()
...
plt.savefig('/home/10714016/gpt/result/combined_data/whole/acc_cloze_smiles_enprop5.jpg')
